...
if not st.session_state.token:
    ...
    # st.link_button is the only Streamlit method that correctly navigates
    # the top-level browser window rather than the sandboxed iframe
    col1, col2, col3 = st.columns([2, 1.4, 2])
    with col2:
        st.link_button("Sign in with Microsoft", url=auth_url, use_container_width=True)
    st.stop()
